...
# ------------------------------------------------------------------------------
#  This software (FMFM) utilizes the HASH_v1.2 for focal mechanism
#  inversion. HASH is a third-party program developed by Hardebeck and Shearer
#  and is subject to its own terms of use.
#
#  Users of this software should cite the original HASH publications:
#
...
#
#  Hardebeck, J. L., & Shearer, P. M. (2003). Using S/P Amplitude Ratios 
#  to Constrain the Focal Mechanisms of Small Earthquakes. Bulletin of the 
#  Seismological Society of America, 93(6), 2434–2444. 
#
# ==============================================================================
...
sys.path.append('../src')
from utils_HASH import run_external_program, HashOutputParser
...
class HashPipeline:
    ...
    def process_and_save_results(self):
        ...
        try:
            ...
            output_dir = os.path.dirname(self.config.hash_fms_csv_file)
            if output_dir:
            ...
            with open(self.config.hash_fms_csv_file, 'w', newline='') as f:
                writer = csv.writer(f)
            ...
            print(f"SUCCESS: Formatted results saved to '{self.config.hash_fms_csv_file}'")
