...
130   continue
     ...
     &       pickonset,pickpol
135     format (a5,a2,2x,a3,1x,a1,1x,a1)
...
30    do 40 i=1,nin
        read (12,35,end=41) sname(k),scomp(k),snet(k),spin
35      format (a5,a3,1x,a2,17x,f10.3)
c        read (12,35,end=41) sname(k),scomp(k),snet(k),
c     &     qns1,qpamp,qns2,qsamp
c35      format (a4,1x,a3,1x,a2,17x,f10.3,1x,f10.3,
c     &           1x,f10.3,1x,f10.3)
        call GETSTAT_SCSN(stfile,sname(k),scomp(k),snet(k),
     &               flat,flon,felv)   ! SCSN station information - ** YOU MAY NEED TO USE YOUR OWN SUBROUTINE **
...
c        s2n1=abs(qpamp)/qns1
c        s2n2=qsamp/qns2
c        spin=qsamp/abs(qpamp)
        if (qcor.eq.-999.) goto 40
c        if (qpamp.eq.0.) goto 40
c        if ((s2n1.lt.ratmin).or.(s2n2.lt.ratmin)) goto 40
c        sp_ratio(k)=log10(spin)-qcor
        sp_ratio(k)=spin-qcor
        p_pol(k)=0
